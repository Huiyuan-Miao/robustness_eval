# to test the model's ability to different types of corruption using dataset imagenet - C
import os
import sys
import random
import time
...
def main(whichModel,valDataset,valCond):
    ...
    #### Train/Val #####################################################################################################
    print(valCond)
    stat_file = open(os.path.join(model_path, 'evalImageNetC'+add_on+'.txt'), 'a+')
    val(val_loader, model, loss_function, optimizer, stat_file, gpu_ids,valCond)
    stat_file.close()
...
def val(val_loader, model, loss_function, optimizer, stat_file, gpu_ids,valCond):
    ...
    for batch_index, (inputs, targets, paths, indices) in enumerate(val_loader):
        ...
        stat_str = 'valCond ({}) Loss ({:.4f}) Accuracy1 ({:.4f}) Accuracy5 ({:.4f})'.\
            format(valCond, loss_sum / (batch_index + 1), num_correct1 / batch_size_sum, num_correct5 / batch_size_sum)
...
if __name__ == '__main__':
    imgCDir = "/media/tonglab/789aa685-f3c0-4b78-afb5-88d669eb7059/Data/ImageNet-C/"
    type = ['noise', 'digital', 'blur', 'extra', 'weather']

    for d in type:
        fs = os.listdir(imgCDir + d)
        for f in fs:
            ls = os.listdir(imgCDir + d + '/' + f)
            for l in [1,2,3,4,5]:
                data = imgCDir + d + '/' + f + '/' + str(l)
                noiseType = d + '_' + f + '_' + str(l)
                # for i in ['ckpt_latest_trainForClassification.pth','ckpt_latest_finetuneWithDualErr.pth']:
                for i in ['ckpt_latest_finetuneWithDualErr.pth']:
                    main(whichModel = i,valDataset=data,valCond=noiseType)
