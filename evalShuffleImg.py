...
from shuffleImage import Shuffler
...
def main(whichModel):
    ...
    #### Train/Val #####################################################################################################
    for dim in [2,4,8]:#[1.  , 0.95, 0.9 , ]
        print(dim)
        stat_file = open(os.path.join(model_path, 'evalShuffleImg'+add_on+'.txt'), 'a+')
        stat_file2 = os.path.join(model_path, 'evalShuffleImgOutput_dim_'+str(dim)+add_on+'.mat')

        val(val_loader, model, loss_function, optimizer, stat_file,stat_file2, gpu_ids,dim)
        stat_file.close()
...
def val(val_loader, model, loss_function, optimizer, stat_file,stat_file2, gpu_ids,dim):
    ...
    for batch_index, (inputs, targets, paths, indices) in enumerate(val_loader):
        inputs = add_shuffle_image(inputs,dim)
        if len(gpu_ids) >= 1:
            inputs = inputs.cuda(non_blocking=True)
            targets = targets.cuda(non_blocking=True)
        ...
        softmax_output_numpy = np.array(nn.Softmax(dim=1)(outputs).detach().cpu())
        target = np.array(targets.detach().cpu())

        correctAns[count:count + inputs.size(0)] = target
        actualAns[count:count + inputs.size(0)] = np.argmax(softmax_output_numpy, axis=1)
        for i in range(inputs.size(0)):
            confCorrectAns[count + i] = softmax_output_numpy[i, int(correctAns[count + i])]
            confActualAns[count + i] = softmax_output_numpy[i, int(actualAns[count + i])]
            ansEntropy[count + i] = scipy.stats.entropy(softmax_output_numpy[i, :])

        if softmax_output_numpy.shape[1] == 1001:
            confNoiseCate[count:count + inputs.size(0)] = softmax_output_numpy[:, -1]
        count = count + inputs.size(0)
        ...
        stat_str = 'dim ({:.4f}) Loss ({:.4f}) Accuracy1 ({:.4f}) Accuracy5 ({:.4f})'.\
            format(dim, loss_sum / (batch_index + 1), num_correct1 / batch_size_sum, num_correct5 / batch_size_sum)
...
def add_shuffle_image(images,dim):
    shuffled_images = torch.zeros_like(images)
    for i in range(images.size(0)):
        image = Shuffler(images[i,:,:,:].permute(1,2,0))
        image.shuffle(matrix=(dim, dim))
        shuffled_images[i] = image.shuffled.permute(2,0,1)
