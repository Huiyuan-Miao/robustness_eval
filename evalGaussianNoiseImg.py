...
def main(whichModel,num_classes,dataDir):
    ...
    model = AlexNet_groupNorm(num_classes=num_classes)
    ...
    for ssnr in [1,0.95,0.9,0.85, 0.8 , 0.75, 0.7 , 0.65, 0.6 , 0.55, 0.5 , 0.45, 0.4 , 0.35, 0.3 , 0.25, 0.2 , 0.15, 0.1 , 0.05,0]:#[1.  , 0.95, 0.9 , ]
        ...
        stat_file2 = os.path.join(model_path, whichModel, 'evalGaussianNoiseImgOutput_SSNR_'+str(int(ssnr*100)) + '.mat')
...
def val(val_loader, model, loss_function, optimizer, stat_file, stat_file2,gpu_ids,ssnr):
    ...
    for batch_index, (inputs, targets, paths, indices) in enumerate(val_loader):
        inputs = add_gauss_noise(inputs,ssnr)
        if len(gpu_ids) >= 1:
            inputs = inputs.cuda(non_blocking=True)
            targets = targets.cuda(non_blocking=True)
